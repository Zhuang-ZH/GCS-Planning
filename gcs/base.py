...
def polytopeDimension(A, b, tol=1e-4):
    """计算凸多面体的维度，通过线性不等式约束来表示，矩阵 A 和向量 b 分别表示这些约束的系数和常数项"""
...
class BaseGCS:
    ...
    def solveGCS(self, rounding, preprocessing, verbose):
        ...
        # best_path：表示找到的最佳路径，是一组边（edges）的集合。它是路径的具体表示形式。
        # best_result：表示求解器返回的最佳结果对象，包含了求解的详细信息，如最优成本、求解时间等
        return best_path, best_result, results_dict
