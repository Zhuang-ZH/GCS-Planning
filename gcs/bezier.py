...
class BezierGCS(BaseGCS):
    """regions(区域)、order（B 样条轨迹的阶数）、continuity（连续性）、edges（边，默认为 None）、hdot_min（最小时间导数，即路径上速度）和 full_dim_overlap（全维度重叠，默认为 False）"""
    # order: B 样条轨迹的阶数，阶数越高，所需的控制点数量越多，平滑性越好，计算复杂度也越高
    # continuity: 路径的连续性，连续性越高，路径的平滑性越好，计算复杂度也越高
    # edges: 边缘，用于定义图中的边
    # hdot_min: 最小时间导数，即路径上速度最小值
    # full_dim_overlap: 全维度重叠，如果为 True，则通过计算区域的全维度重叠来确定图中的边
    def __init__(self, regions, order, continuity, edges=None, hdot_min=1e-6, full_dim_overlap=False):
        ...
        # 创建创建一个由 A_time 和 b_time 定义的多面体
        ...
        # 遍历 self.regions 列表，为每个区域生成一个新的集合，并将其添加到 self.gcs 中
        ...
        # 将输入数据 u_control 和 u_duration 处理成适合 B 样条轨迹计算的格式，并创建两个不同的 B 样条轨迹对象
    ...
    def addPathLengthCost(self, weight):
        """添加路径长度成本"""
        # 通过增加路径长度的成本，路径规划算法会倾向于选择较短的路径，以最小化总成本。
        # 这种方法适用于路径的离散表示，即通过控制点来定义路径。
    ...
    def addPathLengthIntegralCost(self, weight, integration_points=100):
        """根据给定的权重和积分点数，计算路径的导数，并将其转换为成本函数，然后将这些成本函数添加到图的边缘成本中"""
        # 通过积分路径长度的成本，路径规划算法会更精确地评估路径的长度，尤其是在路径的连续性和光滑性方面。
        # 这种方法适用于路径的连续表示，即通过积分点来定义路径。
        if isinstance(weight, float) or isinstance(weight, int):
            weight_matrix = weight * np.eye(self.dimension)
        else:
            assert(len(weight) == self.dimension)
    ...
    def addDerivativeRegularization(self, weight_r, weight_h, order):
        """通过添加导数正则化项来平滑轨迹，从而在优化过程中避免过度拟合"""
        # weight_r 和 weight_h 决定了正则化项的强度，越大轨迹越平滑，而 order 决定了正则化项作用的导数阶数。
        # 如果只需要平滑速度，可以选择 order=2；如果需要平滑加速度，可以选择 order=3。
    ...
    def addVelocityLimits(self, lower_bound, upper_bound):
        """定义每个维度上的速度限制，并通过这些限制构建线性约束，确保系统的速度在指定范围内"""
    ...
    def SolvePath(self, rounding=False, verbose=False, preprocessing=False):
        """通过求解图约束系统 (GCS) 来找到最佳路径，并将该路径转换为贝塞尔轨迹"""
